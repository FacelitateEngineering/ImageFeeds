...
class VideoFeed:
    def __init__(self, display=False, post_processing=[], thread=True, drop_frame=True):
        self.img = None
        self.dirty = True
        self.started = False
...
class IPCamFeed(VideoFeed):
    def __init__(self, url, size=(480, 640), display=False, post_processing=[]):
        """works with the camera app IPCam on play store"""
        super().__init__(display, post_processing)
        self.url = url
        self.height, self.width = size
        self.img = None
    ...
    def _get_image(self):
        try:
            ...
            print(f'Cannot retrieve image from {self.url}')
            img = None
        return img
